      ...
      <AnimatePresence>
        <motion.div {...slideAnimation("down")} className={`w-[30vh] sm:w-[35vh] lg:w-[40vh] h-screen flex flex-col gap-y-3 ${isSunny ? 'bg-DarkSideBarBg' : 'bg-LightSideBarBg'}`}>
          ...
          <form className='flex flex-col items-center justify-center mt-5 mb-5'>
            <motion.input
              {...fadeAnimation}
              className={`${isSunny ? 'bg-InputGray text-white placeholder:text-gray-500' : 'bg-white text-InputGray placeholder:text-gray-500'} w-60  p-2 rounded-md sm:w-[30vh] lg:w-64 2xl:w-[35vh] 2xl:h-12  font-medium outline-none`}
              type='text'
              placeholder='Search projects...'
            />
          </form>
          <motion.p {...fadeAnimation} className={`${isSunny ? 'text-white' : 'text-black'} font-semibold text-2xl sm:text-3xl lg:text-3xl 2xl:text-4xl 2xl:pl-10 text-start pl-5 mt-3 2xl:mb-6 mb-2 sm:mb-4 lg:mb-4 `}>
            Categories
          </motion.p>
          <motion.div {...fadeAnimation} className='flex flex-col gap-y-5 items-center'>
            {categories.map((category, index) => (
              <motion.button
                key={`${category}-${index}`}
                className={`flex w-52 sm:w-[28vh] lg:w-60 2xl:w-[30vh] 2xl:h-14 h-12 font-semibold items-center text-start pl-5 rounded-md ${selectedCategory === category ? 'bg-Green' : ''} ${isSunny ? 'hover:bg-InputGray' : 'hover:bg-gray-400'} `}
